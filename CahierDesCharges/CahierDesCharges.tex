	...
	\section{Contraintes sur le projet}
		...
		\subsection{Glossaire et conventions de dénomination}
			...
			\item \textbf{L’algorithme génétique} est composé de ces trois étapes :
				...
				Il existe plusieurs techniques pour appliquer une mutation. Nous avons choisi une mutation équiprobable basé sur une probabilité très faible qu’un gène subisse une mutation. 
				Cette méthode nous semble être adéquate pour un algorithme génétique générique, puisqu’elle est équiprobable elle est applicable à un grand nombre de domaines d’application et une probabilité très faible 
				permet de modifier un gène d’un individu de manière ponctuel, une trop grande probabilité modifierait trop les individus et fausserait nos résultats.
				\item Le \textbf{crossover} est le croisement de solutions utilisé pour en créer de nouvelles.
				Il existe plusieurs types de crossover applicables aux algorithmes génétique. Pour notre logiciel, nous avons choisi d’appliquer un cross-over “multipoint”. 
				Cette méthode consiste à séparer deux individus parents en plusieurs morceaux, nous avons choisi deux morceaux, et de recombiner ces morceaux pour obtenir un individu enfant. 
				Cette méthode nous semble être le meilleur choix pour un algorithme génétique générique. 
				En effet, elle permet d’obtenir des individus issus des générations précédentes tout en permettant de proposer une meilleure solution au problème traité.
